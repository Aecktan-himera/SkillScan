...
const Navbar = () => {
  ...
  return (
    <nav className={`sticky top-0 z-50 shadow-md transition-all duration-300 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'
      }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden md:ml-10 md:flex md:space-x-8">
              ...
              {user && user.role !== 'admin' && (
                <Link
                  to="/topics"
                  className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium ${isActive('/topics')
                      ? darkMode
                        ? 'border-emerald-500 text-emerald-400'
                        : 'border-emerald-600 text-emerald-700'
                      : 'border-transparent hover:border-gray-300 text-gray-500 hover:text-gray-700'
          ...
          <div className="flex items-center">
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 rounded-full mr-3 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'
                } transition`}
            ...
            {user ? (
              <div className="ml-4 relative flex items-center group">
                <button className="flex text-sm rounded-full focus:outline-none">
                  <div className={`h-8 w-8 rounded-full flex items-center justify-center ${darkMode ? 'bg-emerald-700' : 'bg-emerald-100'
                    }`}>
                ...
                <div className={`origin-top-right absolute right-0 top-full mt-2 w-48 rounded-md shadow-lg py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 ${darkMode ? 'bg-gray-800' : 'bg-white'
      ...
      <div className="md:hidden">
        <div className={`pt-2 pb-3 space-y-1 px-2 ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
          ...
          <Link
            to="/topics"
            className={`flex items-center pl-3 pr-4 py-2 border-l-4 text-base font-medium ${isActive('/topics')
                ? darkMode
                  ? 'border-emerald-500 bg-emerald-900/20 text-emerald-300'
                  : 'border-emerald-600 bg-emerald-50 text-emerald-700'
                : 'border-transparent hover:border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-800'
          ...
          {user && (
            <Link
              to="/history"
              className={`flex items-center pl-3 pr-4 py-2 border-l-4 text-base font-medium ${isActive('/history')
                  ? darkMode
                    ? 'border-emerald-500 bg-emerald-900/20 text-emerald-300'
                    : 'border-emerald-600 bg-emerald-50 text-emerald-700'
                  : 'border-transparent hover:border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-800'
          ...
          {user && user.role === 'admin' && (
            <Link
              to="/admin"
              className={`flex items-center pl-3 pr-4 py-2 border-l-4 text-base font-medium ${isActive('/admin')
                  ? darkMode
                    ? 'border-emerald-500 bg-emerald-900/20 text-emerald-300'
                    : 'border-emerald-600 bg-emerald-50 text-emerald-700'
                  : 'border-transparent hover:border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-800'
